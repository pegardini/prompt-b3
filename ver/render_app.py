...
app = Flask(__name__)
# Necessário para usar sessão de admin (cookie assinado). Gere um valor fixo e
# salve como variável de ambiente SECRET_KEY no Render — se não tiver, gera um
# valor aleatório a cada start, o que faz o admin deslogar a cada deploy/restart.
app.secret_key = os.environ.get('SECRET_KEY', secrets.token_hex(32))

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
PROMPT_FILE = os.path.join(BASE_DIR, 'PROMPT_MESTRE_HIBRIDO_B3_v7.md')

# Sem valor padrão: se ADMIN_SENHA não estiver configurada no Render, o app
# falha ao iniciar em vez de aceitar silenciosamente uma senha visível no código.
...
def prompt_com_chave(chave, duracao_label):
    """Lê o prompt original e substitui o placeholder da chave no cabeçalho."""
    with open(PROMPT_FILE, 'r', encoding='utf-8') as f:
        conteudo = f.read()

    # Substitui o placeholder [CHAVE_DE_LICENCA] no cabeçalho pelo valor real
    if '[CHAVE_DE_LICENCA]' in conteudo:
        novo_conteudo = conteudo.replace('[CHAVE_DE_LICENCA]', chave, 1)
    else:
        # Fallback: insere bloco de chave após os metadados
        bloco_chave = f"""
---
...
> Cole esta chave quando a IA pedir para liberar o acesso completo.
...
@app.route('/admin')
def admin_page():
    # Se já autenticado nesta sessão (cookie assinado), não pede senha de novo.
...
<body>
    <div class="login-box">
        <h1>🔐 Painel Admin</h1>
        <p>Prompt Fundamentalista B3 — Acesso Restrito</p>
        <form method="get" action="/admin">
            <input type="password" name="senha" placeholder="Digite a senha do admin" autofocus>
